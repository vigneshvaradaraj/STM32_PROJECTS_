...
int main(void)
{
  ...
  //Copy in a string
  strncpy((char*)readBuf, "a new file is made by thalapathy vignesh!", strlen("a new file is made by thalapathy vignesh!"));
  UINT bytesWrote;
  fres = f_write(&fil, readBuf, strlen("a new file is made by thalapathy vignesh!"), &bytesWrote);
  if(fres == FR_OK) {
...
static void MX_SPI1_Init(void)
{
  ...
  SD_SPI_HANDLE.Init.NSS = SPI_NSS_HARD_OUTPUT;
